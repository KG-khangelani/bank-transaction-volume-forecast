...
def create_features(data_dir='data/inputs'):
    ...
    # Calculate basic time-window averages
    features = features.with_columns([
        ((pl.col("txn_count_m1") + pl.col("txn_count_m2") + pl.col("txn_count_m3")) / 3).alias("txn_avg_3m"),
        (pl.sum_horizontal([f"txn_count_m{i}" for i in range(1, 7)]) / 6).alias("txn_avg_6m"),
        (pl.sum_horizontal(month_cols) / 12).alias("txn_avg_12m"),
    ])

    # Calculate momentum, volatility and comparative ratios
    features = features.with_columns([
        (pl.col("txn_avg_3m") / (pl.col("txn_avg_6m") + 1e-5)).alias("txn_momentum_3_6"),
        (pl.col("txn_avg_6m") / (pl.col("txn_avg_12m") + 1e-5)).alias("txn_momentum_6_12"),
        ((pl.sum_horizontal([pl.col(f"txn_count_m{i}").pow(2) for i in range(1, 13)]) / 12) 
         - pl.col("txn_avg_12m").pow(2)).abs().sqrt().alias("txn_volatility_12m"),
        (pl.col("txn_count_m1") / (pl.col("target_lag_1yr") + 1e-5)).alias("txn_vs_last_year_ratio"),
    ])
