...
def create_features(data_dir='data/inputs'):
    ...
    features = features.with_columns([
        # Monthly trend slope over M1-M12 using OLS weights (positive = growing activity)
        # Time index: M12=oldest=0, M1=newest=11; weights = index - 5.5; denom = 143.0
        (
            (
                pl.col("txn_count_m12").fill_null(0).log1p() * (-5.5)
                + pl.col("txn_count_m11").fill_null(0).log1p() * (-4.5)
                + pl.col("txn_count_m10").fill_null(0).log1p() * (-3.5)
                + pl.col("txn_count_m9").fill_null(0).log1p() * (-2.5)
                + pl.col("txn_count_m8").fill_null(0).log1p() * (-1.5)
                + pl.col("txn_count_m7").fill_null(0).log1p() * (-0.5)
                + pl.col("txn_count_m6").fill_null(0).log1p() * (0.5)
                + pl.col("txn_count_m5").fill_null(0).log1p() * (1.5)
                + pl.col("txn_count_m4").fill_null(0).log1p() * (2.5)
                + pl.col("txn_count_m3").fill_null(0).log1p() * (3.5)
                + pl.col("txn_count_m2").fill_null(0).log1p() * (4.5)
                + pl.col("txn_count_m1").fill_null(0).log1p() * (5.5)
            ) / 143.0
        ).alias("monthly_trend_slope_12m"),
        # Count of zero-activity months in M1-M12
        (
            pl.col("txn_count_m1").fill_null(0).eq(0).cast(pl.Int32)
            + pl.col("txn_count_m2").fill_null(0).eq(0).cast(pl.Int32)
            + pl.col("txn_count_m3").fill_null(0).eq(0).cast(pl.Int32)
            + pl.col("txn_count_m4").fill_null(0).eq(0).cast(pl.Int32)
            + pl.col("txn_count_m5").fill_null(0).eq(0).cast(pl.Int32)
            + pl.col("txn_count_m6").fill_null(0).eq(0).cast(pl.Int32)
            + pl.col("txn_count_m7").fill_null(0).eq(0).cast(pl.Int32)
            + pl.col("txn_count_m8").fill_null(0).eq(0).cast(pl.Int32)
            + pl.col("txn_count_m9").fill_null(0).eq(0).cast(pl.Int32)
            + pl.col("txn_count_m10").fill_null(0).eq(0).cast(pl.Int32)
            + pl.col("txn_count_m11").fill_null(0).eq(0).cast(pl.Int32)
            + pl.col("txn_count_m12").fill_null(0).eq(0).cast(pl.Int32)
        ).alias("zero_activity_months_12m"),
        # Year-over-year log difference for most recent month: Oct 2015 vs Oct 2014
        (
            pl.col("txn_count_m1").fill_null(0).log1p()
            - pl.col("txn_count_m13").fill_null(0).log1p()
        ).alias("yoy_m1_log_diff"),
        # Same-calendar-window comparison: last year's target period vs current recent 3m
        (
            pl.col("target_lag_1yr").fill_null(0).log1p()
            - pl.col("txn_count_last_3m").fill_null(0).log1p()
        ).alias("lag_vs_recent_log_diff"),
        # Seasonal pre-target level: Nov 2014 (M12) relative to Oct 2015 (M1)
        (
            pl.col("txn_count_m12").fill_null(0).log1p()
            - pl.col("txn_count_m1").fill_null(0).log1p()
        ).alias("lag_nov_vs_current_oct"),
    ])
