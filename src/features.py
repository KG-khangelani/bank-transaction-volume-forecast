...
def create_features(data_dir='data/inputs'):
    ...
    features = features.with_columns([
        ...
        # Measures the absolute difference in transaction volume for the target month 
        # across the last two years. This provides a linear signal of how consistent 
        # a user's seasonal peak is, without using a high-variance ratio.
        (pl.col("target_lag_1yr").fill_null(0) - pl.col("target_lag_2yr").fill_null(0)).abs().alias("seasonal_stability_abs_diff"),

        # 2. Monthly Volume Range:
        # Computes the range (Max - Min) of monthly transaction counts over the last year.
        # This identifies "spiky" vs "steady" users using a simple additive signal,
        # avoiding the volatility of standard deviation or coefficient of variation.
        (
            pl.max_horizontal([f"txn_count_m{i}" for i in range(1, 13)]) - 
            pl.min_horizontal([f"txn_count_m{i}" for i in range(1, 13)])
        ).alias("monthly_volume_range_12m"),

        # 3. Log-Scaled Ticket Size:
        # Captures the average value per transaction. Using log1p to compress 
        # the monetary variance is critical for RMSLE stability.
        (
            (pl.col("txn_amount_sum_last_3m").abs().fill_null(0) / (pl.col("txn_count_last_3m").fill_null(0) + 1))
            .log1p()
        ).alias("log_ticket_size_3m"),

        # 4. Financial Magnitude Anchors:
        # Providing log-scaled anchors for wealth and engagement.
        # These provide the model with a baseline scale for the user's financial footprint
        # without introducing high-variance ratio noise.
        pl.col("stmt_balance_mean_3m").abs().fill_null(0).log1p().alias("log_balance_anchor_3m"),
        (pl.col("fin_interest_income_mean").fill_null(0) + pl.col("fin_interest_revenue_mean").fill_null(0)).log1p().alias("log_fin_engagement_anchor")
    ])
