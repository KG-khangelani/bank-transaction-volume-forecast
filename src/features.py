...
def create_features(data_dir='data/inputs'):
    ...
    # Give more weight to the most recent months to capture current behavior trends
    ...
    # High volatility in monthly counts often suggests sporadic usage
    features = features.with_columns([
    ...
    # Compare current month to previous months and the same month last year
    features = features.with_columns([
    ...
    # Combine demographics or account status with activity
    features = features.with_columns([
    ...
    # 5. Log transforms for highly skewed continuous variables
    # This helps the GBDT split more effectively on large-scale values
