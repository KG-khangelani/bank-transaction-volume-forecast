...
def create_features(data_dir='data/inputs'):
    ...
    def generate_features(df):
        ...
        # 2. Lags (Crucial for transaction forecasting)
        lags = [1, 7, 14]
        for lag in lags:
            df = df.with_columns(
                pl.col("txn_count").shift(lag).over("account_id")
                .fill_null(strategy="forward").fill_null(0).alias(f"lag_{lag}")
            )

        # 3. Rolling Window Statistics (on log-transformed target to stabilize variance)
        # We use log1p to handle zeros and reduce the impact of extreme outliers
        df = df.with_columns(
            (pl.col("txn_count") + 1).log().alias("log_txn")
        )

        df = df.with_columns([
            pl.col("log_txn").shift(1).rolling_mean(window_size=7).over("account_id").fill_null(0).alias("rolling_log_mean_7"),
            pl.col("log_txn").shift(1).rolling_mean(window_size=28).over("account_id").fill_null(0).alias("rolling_log_mean_28"),
            pl.col("txn_count").shift(1).rolling_std(window_size=7).over("account_id").fill_null(0).alias("rolling_std_7"),
        ])

        # 4. Trend and Momentum Features
        df = df.with_columns([
            (pl.col("lag_1") / (pl.col("lag_7") + 1)).alias("momentum_1_7"),
            (pl.col("lag_1") - pl.col("lag_7")).alias("diff_1_7"),
        ])

        # 5. Account-level profile (Static features)
        df = df.with_columns(
            pl.col("txn_count").mean().over("account_id").alias("account_avg_vol"),
            pl.col("txn_count").max().over("account_id").alias("account_max_vol"),
        )

        return df.drop("log_txn")
