...
def create_features(data_dir='data/inputs'):
    ...
    # The model exhibits a strong preference for raw counts and behavioral intensities 
    # (counts per active day) but is highly sensitive to high-variance ratios.
    #
    # To improve RMSLE, we pivot to "Linear Seasonal Drifts" and "Log-Scaled Magnitude Anchors".
    # We avoid division-based noise and multiplicative growth signals, focusing instead 
    # on linear gaps and log-compressed financial signals to provide scale.
    #
    # 1. Seasonal Gap: A linear difference between the historical target month volume 
    #    and the recent 3-month average. This signals if the user is entering a seasonal 
    #    peak or trough relative to their current run-rate without the variance of a ratio.
    # 2. Balance Magnitude: The log-absolute of the mean balance. This provides a 
    #    non-ratio financial scale anchor.
    # 3. Financial Engagement: An additive combination of interest income and revenue, 
    #    log-transformed to compress variance. This captures the user's financial 
    #    footprint using untried dimensions.
    # 4. Transaction Reliability: A simple additive sum of bounded failure ratios 
    #    (reversals and bounces), providing a quality signal.

    features = features.with_columns([
        # 1. Seasonal Gap: (target_lag_1yr - recent_avg)
        (
            pl.col("target_lag_1yr").fill_null(0) - 
            (pl.col("txn_count_last_3m").fill_null(0) / 3)
        ).alias("seasonal_runrate_gap"),

        # 2. Balance Magnitude: Log-scaled absolute mean balance
        (
            pl.col("stmt_balance_mean_3m").abs().fill_null(0).log1p()
        ).alias("log_balance_magnitude_3m"),

        # 3. Financial Engagement: log1p(income + revenue)
        (
            (pl.col("fin_interest_income_mean").fill_null(0) + 
             pl.col("fin_interest_revenue_mean").fill_null(0))
            .log1p()
        ).alias("log_fin_engagement_magnitude"),

        # 4. Transaction Reliability: sum of bounded ratios
        (
            pl.col("reversal_ratio").fill_null(0) + 
            pl.col("bounced_ratio").fill_null(0)
        ).alias("txn_failure_combined_score")
    ])
