...
def create_features(data_dir='data/inputs'):
    ...
    # Define monthly columns for easier reference
    month_cols = [f"txn_count_m{i}" for i in range(1, 13)]

    # 1. Recency-Weighted Averages (Extended)
    features = features.with_columns([
        (pl.col("txn_count_m1") * 0.5 + pl.col("txn_count_m2") * 0.3 + pl.col("txn_count_m3") * 0.2).alias("txn_weighted_avg_3m"),
        (pl.col("txn_count_m1") * 0.4 + pl.col("txn_count_m2") * 0.3 + pl.col("txn_count_m3") * 0.2 + pl.col("txn_count_m4") * 0.1).alias("txn_weighted_avg_4m"),
    ])

    # 2. Stability and Volatility
    features = features.with_columns([
        (pl.sum_horizontal([pl.col(f"txn_count_m{i}").pow(2) for i in range(1, 13)]) / 12 
         - (pl.sum_horizontal(month_cols) / 12).pow(2)).abs().sqrt().alias("txn_std_12m"),
    ])
    features = features.with_columns([
        (pl.col("txn_std_12m") / (pl.sum_horizontal(month_cols) / 12 + 1e-5)).alias("txn_cv_12m"),
    ])

    # 3. Relative Deltas and Growth
    features = features.with_columns([
        (pl.col("txn_count_m1") - pl.col("txn_count_m2")).alias("txn_delta_m1_m2"),
        (pl.col("txn_count_m1") / (pl.col("txn_count_m2") + 1e-5)).alias("txn_ratio_m1_m2"),
        (pl.col("txn_count_m1") - pl.col("target_lag_1yr")).alias("txn_delta_yoy"),
        (pl.col("target_lag_1yr") / (pl.col("target_lag_2yr") + 1e-5)).alias("lag_growth_ratio"),
    ])

    # 4. Momentum and Windowed Trends
    features = features.with_columns([
        ((pl.col("txn_count_m1") + pl.col("txn_count_m2") + pl.col("txn_count_m3")) / 3 
         - (pl.col("txn_count_m4") + pl.col("txn_count_m5") + pl.col("txn_count_m6")) / 3).alias("txn_momentum_3m_vs_6m"),
        ((pl.col("txn_count_m1") + pl.col("txn_count_m2") + pl.col("txn_count_m3")) / 
         ((pl.col("txn_count_m4") + pl.col("txn_count_m5") + pl.col("txn_count_m6")) + 1e-5)).alias("txn_growth_ratio_3m_vs_6m"),
    ])

    # 5. Log-transformed Monthly Counts
    features = features.with_columns([
        pl.col(f"txn_count_m{i}").log1p().alias(f"log_txn_m{i}") for i in range(1, 7)
    ])

    # 6. Interaction Features
    features = features.with_columns([
        (pl.col("txn_count_m1") * pl.col("target_lag_1yr")).alias("activity_interaction_recent_lag"),
        (pl.col("AnnualGrossIncome") * pl.col("txn_count_all")).alias("income_activity_interaction"),
        (pl.col("target_lag_1yr") * pl.col("txn_momentum_3m_vs_6m")).alias("lag_momentum_interaction"),
    ])

    # 7. Log transforms for highly skewed continuous variables
    skewed_cols = ["AnnualGrossIncome", "txn_amount_sum_all", "txn_amount_sum_last_1m", "txn_amount_sum_last_3m"]
    features = features.with_columns([
        pl.col(col).log1p().alias(f"log_{col}") for col in skewed_cols if col in features.columns
    ])

    # --- NEW ADDITIONS ---

    # 8. Sparsity and Activity Patterns
    features = features.with_columns([
        pl.sum_horizontal([pl.col(c).eq(0) for c in month_cols]).alias("zero_txn_months_count"),
        (pl.sum_horizontal([pl.col(c).gt(0) for c in month_cols]) / 12).alias("activity_consistency_rate"),
    ])

    # 9. Year-over-Year Ratio (Target Specific)
    features = features.with_columns([
        ((pl.col("txn_count_m1") + 1) / (pl.col("target_lag_1yr") + 1)).alias("txn_ratio_m1_vs_lag1"),
        ((pl.col("txn_count_m1") + 1) / (pl.col("target_lag_2yr") + 1)).alias("txn_ratio_m1_vs_lag2"),
    ])

    # 10. Short-term vs Long-term Trend
    features = features.with_columns([
        (pl.col("txn_count_m1") / (pl.sum_horizontal(month_cols)/12 + 1e-5)).alias("recent_vs_annual_avg"),
        ((pl.col("txn_count_m1") + pl.col("txn_count_m2") + pl.col("txn_count_m3")) / 3 / (pl.sum_horizontal(month_cols)/12 + 1e-5)).alias("q_recent_vs_annual_avg"),
    ])

    # 11. Volume Stability / Drift
    features = features.with_columns([
        (pl.col("txn_count_m1") - pl.col("txn_count_m6")).alias("txn_drift_6m"),
        (pl.col("txn_count_m1") - pl.col("txn_count_m12")).alias("txn_drift_12m"),
    ])
