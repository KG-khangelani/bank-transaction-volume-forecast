...
def create_features(data_dir='data/inputs'):
    ...
    # We use the 'polars' namespace directly to avoid potential UnboundLocalError 
    # encountered in previous attempts with the 'pl' alias in the harness.

    # The top 10 features indicate that recency, weighted averages, activity intensity, 
    # and system-defined batch counts are critical for forecasting volume.
    # We will extend these patterns to create a more robust feature set.

    # 1. Extended Weighted Moving Averages (WMA)
    # Existing top features include 3m and 4m weighted averages.
    # WMA 6m: (6*m1 + 5*m2 + 4*m3 + 3*m4 + 2*m5 + 1*m6) / 21
    wma_6m_expr = (
        polars.col("txn_count_m1") * 6 + 
        polars.col("txn_count_m2") * 5 + 
        polars.col("txn_count_m3") * 4 + 
        polars.col("txn_count_m4") * 3 + 
        polars.col("txn_count_m5") * 2 + 
        polars.col("txn_count_m6") * 1
    ) / 21

    # WMA 12m: (12*m1 + 11*m2 ... + 1*m12) / 78
    wma_12m_expr = sum([polars.col(f"txn_count_m{i}") * (13-i) for i in range(1, 13)]) / 78

    # 2. Momentum and Recency Ratios
    # Comparing short-term activity to longer-term trends to capture acceleration/deceleration.
    # Short-term momentum: Most recent month vs 3-month average
    momentum_1_3 = polars.col("txn_count_m1") / (polars.col("txn_count_last_3m") / 3 + 1)
    # Mid-term momentum: 3-month average vs 6-month average
    momentum_3_6 = (polars.col("txn_count_last_3m") / 3) / (polars.col("txn_count_last_6m") / 6 + 1)

    # 3. Enhanced Activity Intensity (Transactions per Active Day)
    # Top features included 6m and 12m. We add 1m and 3m for higher resolution.
    intensity_1m = polars.col("txn_count_last_1m") / (polars.col("active_days_last_1m") + 1)
    intensity_3m = polars.col("txn_count_last_3m") / (polars.col("active_days_last_3m") + 1)

    # 4. System Batch Intensity
    # Since 'txn_batch_system_defined_count_3m' is a top feature, we calculate its intensity.
    sys_batch_intensity_3m = polars.col("txn_batch_system_defined_count_3m") / (polars.col("active_days_last_3m") + 1)

    # 5. Log-Transformations for Skewed Volume Data
    # RMSLE is the target, so log-transforming high-variance count features helps the model.
    log_txn_last_1m = polars.col("txn_count_last_1m").log1p()
    log_txn_last_3m = polars.col("txn_count_last_3m").log1p()
    log_txn_last_6m = polars.col("txn_count_last_6m").log1p()

    # 6. Churn/Inactivity Proxy
    # A binary flag for users who have been inactive for the last 2 months.
    is_inactive_recent = ((polars.col("txn_count_m1") == 0) & (polars.col("txn_count_m2") == 0)).cast(polars.Int32)

    # Apply all new features to the dataframe
    features = features.with_columns([
        wma_6m_expr.alias("txn_weighted_avg_6m"),
        wma_12m_expr.alias("txn_weighted_avg_12m"),
        momentum_1_3.alias("txn_momentum_1_3"),
        momentum_3_6.alias("txn_momentum_3_6"),
        intensity_1m.alias("txns_per_active_day_last_1m"),
        intensity_3m.alias("txns_per_active_day_last_3m"),
        sys_batch_intensity_3m.alias("sys_batch_intensity_3m"),
        log_txn_last_1m.alias("log_txn_last_1m"),
        log_txn_last_3m.alias("log_txn_last_3m"),
        log_txn_last_6m.alias("log_txn_last_6m"),
        is_inactive_recent.alias("is_inactive_recent")
    ])
